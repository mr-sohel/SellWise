...
old_file = "SDP300-SellWise(AI Sales Analytics)-FinalReport.md"

with open(old_file, "r", encoding="utf-8") as f:
    old_content = f.read()

def get_code_block(marker):
    pattern = r"```" + marker + r".*?```"
    matches = re.findall(pattern, old_content, re.DOTALL)
    return matches

csharp_blocks = get_code_block("csharp")
python_blocks = get_code_block("python")
json_blocks = get_code_block("json")
sql_blocks = get_code_block("sql")
html_blocks = get_code_block("html")
diagram_blocks = get_code_block("") # For diagrams

# Let's just create a completely fresh markdown file with the structure and humanized text
...
""" + "```python\n" + (python_blocks[1] if len(python_blocks) > 1 else "") + "\n```\n" + """
...
with open("SDP300-SellWise(AI Sales Analytics)-FinalReport.md", "w", encoding="utf-8") as f:
    f.write(new_content)
