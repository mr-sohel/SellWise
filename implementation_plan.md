...
> 5. **npm workspaces monorepo** with shared type definitions across frontend/backend.
> 6. **Category Presets** — Category-focused onboarding: users select what they sell (product categories). System auto-detects business type and pre-seeds categories. No more asking "what kind of business?" — just "what do you sell?"
> 7. **Category System** — Predefined categories per preset, seeded on onboarding, with CRUD API and CategoryPicker UI component.
> 8. **Quick Order Entry** — Search-as-you-type product picker and customer phone lookup for fast order creation by Facebook/WhatsApp sellers.
...
| Monorepo | Flat directories | **npm workspaces** | Proper dependency isolation, shared packages, consistent tooling across all services |
...
| Business adaptation | One-size-fits-all | **Category Presets** | Category-focused onboarding with auto-detected business type |
| Product categories | Free-text only | **Category System** | Predefined categories per preset, seeded on onboarding, with CRUD API and CategoryPicker UI |

---

## 2B. Business Type System

### Overview

SellWise adapts to different business types through a **category-focused onboarding wizard**. After signup, users select what they sell (product categories), and the system auto-detects the business type and pre-seeds relevant categories.

### Business Types

| Business Type | Description | Default Categories | ML Seasonality |
|---------------|-------------|-------------------|----------------|
| `facebook_seller` | Facebook/WhatsApp product sellers | Electronics, Accessories, Audio, Clothing, Beauty, Home & Kitchen | Sale-day spikes (11.11, 12.12), yearly seasonality |
| `small_shop` | Neighborhood grocery/super shops | Grocery, Snacks & Beverages, Dairy & Eggs, Household, Personal Care, Stationery | Weekly (Friday rush), yearly, monsoon dips |
| `online_store` | Shopify/WooCommerce online stores | Electronics, Clothing, Beauty, Home & Kitchen, Sports, Books | Yearly seasonality, weekly patterns |
| `wholesaler` | B2B wholesale distributors | General Merchandise (customizable) | Yearly seasonality, bulk order patterns |

### Sales Channels

| Channel | Description |
|---------|-------------|
| `facebook` | Facebook Messenger orders |
| `whatsapp` | WhatsApp orders |
| `walk_in` | Walk-in / in-person orders |
| `website` | Website / online orders |
| `other` | Other channels |

### Implementation

**Shared Constants** (`packages/shared/src/constants/business.ts`):
```typescript
...
```
**Store Model** — `business_type` is auto-detected from category presets:
...
**Auth Service** — Returns full `store` object (not just `storeId`):
```typescript
// authService.signup() and login() return:
{ user, store, role, token }  // store is a full Store object
```

**Onboarding Flow**:
1. User signs up → auto-created store with `business_type: null`
2. Frontend redirects to `/onboarding`
3. Single page: "What do you sell?" — select product category presets
4. On completion: `PATCH /stores/:storeId/onboarding` accepts `categoryPresetIds`, auto-detects `business_type`, seeds categories
5. `MainLayout` guard checks `store.business_type` — if null, redirects to `/onboarding`
...
Categories are stored in a dedicated `categories` table, scoped per store. Default categories are seeded from category presets during onboarding.
...
| Method | Endpoint | Description |
|--------|----------|-------------|
| `GET` | `/api/v1/stores/:storeId/categories` | List categories for store |
...
**CategoryPicker** (`packages/client/src/features/categories/CategoryPicker.tsx`):
- Dropdown with existing categories from API
- Free-text entry for custom categories
- "Use [custom name]" option for new categories
- Used in CreateProductPage and EditProductPage

**useCategories hook** (`packages/client/src/features/categories/hooks/useCategories.ts`):
- `useCategories(storeId)` — fetches categories for store
- `useCreateCategory(storeId)` — creates new category

---

## 2D. Quick Order Entry

### Problem

Facebook/WhatsApp sellers receive orders via messages like "2 ta phone case, 1 ta earbuds". The previous dropdown-based product selection was too slow for this workflow.

### Solution

**Search-as-you-type product picker**:
- Text input that filters products by name, SKU, or category
- Shows product name, price, and stock in search results
- Click to add to order (fills empty slot or appends)

**Customer phone lookup**:
- Type 3+ characters to search existing customers
- Auto-fill name, address, email when customer is found
- Reduces re-entry for repeat buyers

**Dynamic source dropdown**:
- Shows only the store's configured sales channels
- Replaces the hardcoded "Manual Entry" / "Facebook" options

### Implementation

- Replaced `<select>` dropdown with search input in `CreateOrderPage.tsx`
- Added debounced customer search via `/stores/:storeId/customers` API
- Source dropdown populated from `store.sales_channels` via auth store (if configured)
...
| uvicorn | ASGI server |
| prophet | Time-series demand forecasting with business-type-aware seasonality |
| scikit-learn | Churn prediction (logistic regression) |
| pandas / numpy | Data manipulation |
| psycopg2-binary | PostgreSQL from Python |
| pydantic | Request/response validation (includes `business_type` field) |
...
├── 1783010613861_create-users-and-stores.ts
├── 1783010614299_create-api-keys.ts
├── 1783010614727_create-products.ts
├── 1783010615160_create-customers.ts
├── 1783010615588_create-orders-and-items.ts
├── 1783010616014_create-expenses.ts
├── 1783010616435_create-forecasts.ts
...
| `stores` | id, name, name_bn, currency, timezone, business_type, sales_channels | Business/store entities (business_type auto-detected from categories) |
| `store_members` | store_id, user_id, role (`owner`/`manager`) | RBAC membership |
| `categories` | id, store_id, name, name_bn, is_default, sort_order | Store-scoped product categories (seeded per business type) |
...
### 9.9 Demand Forecasting Dashboard
...
sequenceDiagram
    ...
    participant Controller as AnalyticsController
    participant Service as ForecastService
    participant ForecastRepo as ForecastRepository
    participant ProductRepo as ProductRepository
    participant DB as PostgreSQL

    Seller->>Frontend: Opens Dashboard
    Frontend->>Controller: GET /api/v1/stores/:storeId/analytics/demand-forecast?limit=5&days=30

    Controller->>Service: getDemandForecast(storeId, limit, days)
    Service->>ProductRepo: findTopProductsByOrderCount(storeId, 90)
    ProductRepo->>DB: SELECT product_id, COUNT(*) as order_count, SUM(quantity) as total_units FROM order_items ... GROUP BY product_id ORDER BY order_count DESC LIMIT $1
    ProductRepo-->>Service: Top N products with order stats

    loop For each top product
        Service->>ForecastRepo: findByProductAndDateRange(storeId, productId, today, today+days)
        ForecastRepo->>DB: SELECT * FROM forecasts WHERE store_id=$1 AND product_id=$2 AND forecast_date BETWEEN $3 AND $4
        ForecastRepo-->>Service: Forecast rows
    end

    Service-->>Controller: { top_products: [{ product, forecast: [{ date, predicted_qty, lower_bound, upper_bound }] }] }
    Controller-->>Frontend: ApiResponse.success(demandForecast)
    Frontend->>Frontend: Render DemandForecastChart (day selector + product cards + sparklines)
```

**Demand Forecast API:**
- `GET /api/v1/stores/:storeId/analytics/demand-forecast?limit=5&days=30`
- `limit` (default 5): top N products by order frequency (90-day window)
- `days` (default 30): forecast horizon (7, 15, or 30)

**Implementation:**
- `forecast.repository.ts` → `findTopProductsForecasts()`: joins top products with forecast data
- `forecast.service.ts` → `getDemandForecast()`: orchestrates product lookup + forecast retrieval
- `analytics.controller.ts` → `getDemandForecast`: HTTP boundary
- `analytics.routes.ts` → route added under analytics routes

**Frontend:**
- `useDemandForecast()` hook in `useDashboard.ts`
- `DemandForecastChart.tsx` with:
  - Day selector pill buttons (7D / 15D / 30D)
  - Per-product cards sorted by predicted daily demand
  - Mini sparkline area charts per product
  - Stock vs demand comparison

**Critical: PostgreSQL Numeric Casting:**
PostgreSQL returns numeric/decimal columns as JavaScript strings. Always cast with `Number()`:
```typescript
...
### 9.10 Seed Data & Forecast Generation

```bash
# Generate realistic Bangladeshi electronics seed data
...
**Seed data includes:**
- 51 products across 5 categories (Mobile Phones, Phone Accessories, Computers & Laptops, Audio & Speakers, Cameras)
- 40 Bangladeshi customers with realistic names, phones, addresses
- 800 orders spread over 12 months with seasonal patterns
- 192 expenses across 6 categories
...
│   │   │   │   │       └── useAuth.ts
│   │   │   │   ├── onboarding/
│   │   │   │   │   └── OnboardingPage.tsx     # Category presets selection
│   │   │   │   ├── dashboard/
│   │   │   │   │   ├── DashboardPage.tsx
│   │   │   │   │   └── hooks/
│   │   │   │   │       └── useDashboard.ts
...
/login                          → LoginPage
/signup                         → SignupPage
/onboarding                     → OnboardingPage (category presets)

/dashboard                      → DashboardPage
/products                       → ProductListPage
  └── /new                      → CreateProductPage
  └── /:id/edit                 → EditProductPage
  └── /import                   → BulkImportPage
/orders                         → OrderListPage
  └── /new                      → CreateOrderPage (search-as-you-type)
...
| PATCH | `/api/v1/stores/:storeId/onboarding` | Owner |
| PATCH | `/api/v1/stores/:storeId/profile` | Owner |

### Categories
| Method | Endpoint | Auth |
|--------|----------|------|
| GET | `/api/v1/stores/:storeId/categories` | Owner/Manager |
| POST | `/api/v1/stores/:storeId/categories` | Owner/Manager |
| PATCH | `/api/v1/stores/:storeId/categories/:categoryId` | Owner/Manager |
| DELETE | `/api/v1/stores/:storeId/categories/:categoryId` | Owner |
...
| GET | `/api/v1/stores/:storeId/analytics/demand-forecast?limit=5&days=30` | Owner/Manager |
